...
interface IPlayerRepository: JpaRepository<Player, UUID> {
    @Query("SELECT new com.esosa.f5pi_backend.controllers.responses.PlayerStatisticsResponse("
            + "COALESCE(CAST(COUNT(g) AS long), 0), "
            + "COALESCE(CAST(SUM(m.goalsScored) AS long), 0), "
            + "COALESCE(CAST(SUM(m.ownGoals) AS long), 0), "
            + "COALESCE(CAST(SUM(CASE WHEN t.result = 'WIN' THEN 1 ELSE 0 END) AS long), 0), "
            + "COALESCE(CAST(SUM(CASE WHEN t.result = 'DRAW' THEN 1 ELSE 0 END) AS long), 0), "
            + "COALESCE(CAST(SUM(CASE WHEN t.result = 'LOSS' THEN 1 ELSE 0 END) AS long), 0), "
            + "COALESCE(CAST(SUM(g.individualPrice) AS long), 0)) "
            + "FROM Player p "
